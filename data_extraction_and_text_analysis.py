...
# Preprocess text: Remove stop words and non-alphabetic characters
preprocessed_docs = []
for tokens in docs:
...
sentiment_dict = {}
with open(os.path.join(sentment_dir, 'Output_Data.csv'), 'r', encoding='ISO-8859-1') as f:
    for line in f:
        word, sentiment = line.split(',')
        sentiment_dict[word.lower()] = int(sentiment)
...
# Analyze sentiments for all documents
sentiment_scores = []
for tokens in preprocessed_docs:
    sentiment_score = analyze_sentiment(tokens, sentiment_dict)
    sentiment_scores.append(sentiment_score)

# Save results to CSV
output_df = pd.DataFrame({
    'FileName': os.listdir(text_dir),
